...
1. Agent 容器啟動 → 自動向 Hub `POST /register` 註冊（帶 name、description、url、route_patterns、priority）
2. Agent 每 10 秒發送 heartbeat → Hub 追蹤在線狀態
3. Hub 重啟 → Agent 的 heartbeat 收到 404 → 自動重新註冊
4. Agent 離線 → heartbeat 超時 → Hub 標記為離線，不再路由

### Agent 狀態

| 狀態 | Dashboard 顯示 | 路由可選取 | 觸發條件 |
|------|---------------|-----------|---------|
| **online** | 綠色 | 是 | 正常註冊且認證通過 |
| **unauthenticated** | 顯示錯誤訊息 | 否 | LLM 未認證（無 API key 且未 CLI login） |
| **error** | 紅色 | 否 | agent 啟動失敗回報 |
| **disabled** | 黃色 | 否 | 手動停用 |

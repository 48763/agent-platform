...
| **TG Transfer Agent** | Telegram 群組資源搬移、媒體去重、搜尋、標籤管理 | `tg-transfer-agent` |
...
- **狀態顯示** — 在線 / 離線 / 已停用（顏色區分）
- **統計數據** — 優先級、處理任務數、成功率、平均回應時間、在線時長、最後心跳
- **關鍵字標籤** — 每個 route pattern 獨立顯示
- **停用/啟用** — 停用的 agent 不參與路由，但不移除
- **收合/展開** — 點擊標題可收合區塊
- **搜尋** — 按 agent 名稱、描述、關鍵字篩選

### 對話紀錄

- **Tab 篩選** — 全部 / 處理中 / 已完成 / 已封存 / 已關閉
- **搜尋** — 按對話內容、agent 名稱篩選
- **顯示** — 處理的 agent、來源通訊軟體、訊息數、更新時間、最近對話內容
...
│   │   └── __main__.py            #     入口
│   └── tg_transfer/               #   Telegram 資源搬移 agent
│       ├── Dockerfile
│       ├── agent.yaml             #     路由、優先級、v2 設定
│       ├── __main__.py            #     入口 + dispatch + 搜尋/統計
│       ├── parser.py              #     TG 連結解析、轉發偵測、意圖分類
│       ├── chat_resolver.py       #     群組名稱/連結 → entity
│       ├── db.py                  #     搬移 Job/Message SQLite
│       ├── media_db.py            #     媒體資產/標籤/搜尋 SQLite
│       ├── transfer_engine.py     #     下載/上傳/album/去重/批量
│       ├── hasher.py              #     SHA-256 + pHash 計算
│       ├── tag_extractor.py       #     #tag 提取
│       ├── search.py              #     關鍵字/以圖搜尋 + 翻頁格式化
│       ├── liveness_checker.py    #     背景存活檢查
│       ├── dashboard.py           #     統計 HTML 頁面（/dashboard）
...
│
│
├── data/tg_transfer/              # TG Transfer 持久化（gitignore）
│   ├── transfer.db                #   搬移記錄 + 媒體資產 DB
│   ├── tg_transfer.session        #   Telethon 登入 session
│   └── tmp/                       #   媒體暫存（自動清理）
│
├── tests/                         # 測試（74 tests）
├── docker-compose.yaml
...
Telegram 群組間資源搬移工具，支援：

**搬移功能：**
- 貼 TG 訊息連結 → 自動下載並上傳到預設目標群組
- 轉發訊息 → 自動轉存到目標群組
- 批量搬移 → 自然語言指令（AI 解析），支援時間區間/數量/全部篩選
- 斷點續傳 → 中斷後自動從上次進度繼續
- 跨 Job 去重 → 同一對群組不會重複搬移

**媒體管理（v2）：**
- SHA-256 精確去重 + pHash 感知相似去重
- 關鍵字搜尋（caption + 標籤，每頁 10 筆翻頁）
- 以圖搜圖（pHash Hamming distance 比對）
- 標籤系統（自動提取 #tag，搬移時沿用保存）
- 統計 Dashboard（`/dashboard` endpoint — 儲存數量、標籤統計）
- 資源存活檢查（定期檢查目標訊息是否存在，不存在則清除）

**設定（agent.yaml settings）：**
...
# 進入容器後會提示輸入手機號碼 → 驗證碼 → 完成後 session 持久化
```

**使用方式（在 Telegram 對 bot 說）：**
